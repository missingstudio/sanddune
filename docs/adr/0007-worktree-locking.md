...
Lock files live in the **config directory** at `.sanddune/locks/<name>.lock`, where `<name>` matches the **worktree** directory name under `.sanddune/worktrees/<name>/`. This keeps locks separate from **worktree** contents (invisible to the **agent**) and provides a 1:1 mapping between **worktrees** and locks.
...
- **Lock inside the worktree** (e.g. `<worktree-path>/.sanddune.lock`). Visible to the **agent**, which could delete or commit it. Would require `.gitignore` management.
...
- The `.sanddune/locks/` directory becomes a new managed directory in the **config directory**.
