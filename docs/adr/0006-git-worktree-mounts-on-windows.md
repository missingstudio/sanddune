...
repo itself is a worktree** and when sanddune **creates a worktree** via the
...
1. **Mount the parent `.git` dir at a deterministic POSIX path** —
   `/.sanddune-parent-git`. This gives the parent git directory a stable,
...
2. **Create a corrected `.git` file** with the `gitdir:` path rewritten to
   ...
   `gitdir: /.sanddune-parent-git/worktrees/abc`. Mount this file at
...
5. Remaps the parent `.git` dir mount to `/.sanddune-parent-git`.
...
- **sanddune-created worktree** — adds a new overlay mount, since the `.git`
...
- **Entrypoint script**: Have the container's init script detect and rewrite
  Windows paths. Rejected because it pushes sanddune-specific logic into the
...
- A small temp file is created per sandbox session. It is cleaned up when the
  ...
  normal temp cleanup.
- The `/.sanddune-parent-git` path is reserved inside the sandbox. This is
