...
1. You invoke agents with a single `sanddune.run()`.
2. sanddune handles sandboxing the agent with a configurable branch strategy.
...
- A sandbox provider — sanddune needs an isolated environment to run agents in. Built-in options:
...
2. Run `sanddune init`. This scaffolds a `.sanddune` directory with all the files needed.
...
4. Run the `.sanddune/main.ts` (or `main.mts`) file with `npx tsx`

```bash
npx tsx .sanddune/main.ts
```
...
sanddune uses a `SandboxProvider` to create isolated environments. The `sandbox` option on `run()` and `createSandbox()` accepts any provider. A no-sandbox option is also available for `interactive()` and `wt.interactive()`. Built-in providers:

| Provider   | Import path                                | Type       | Accepted by                                   |
| ---------- | ------------------------------------------ | ---------- | --------------------------------------------- |
| Docker     | `@missingstudio/sanddune/sandboxes/docker`     | Bind-mount | `run()`, `createSandbox()`, `interactive()`   |
| Podman     | `@missingstudio/sanddune/sandboxes/podman`     | Bind-mount | `run()`, `createSandbox()`, `interactive()`   |
| Vercel     | `@missingstudio/sanddune/sandboxes/vercel`     | Isolated   | `run()`, `createSandbox()`, `interactive()`   |
...
sanddune exports a programmatic `run()` function for use in scripts, CI pipelines, or custom tooling. The examples below use `docker()`, but any `SandboxProvider` works in its place.
...
const result = await run({
  ...
  // .sanddune/ artifacts (worktrees, logs, env, patches) and git operations.
  ...
  promptFile: ".sanddune/prompt.md", // path to a prompt file
  ...
  // How to record progress. Default: write to a file under .sanddune/logs/
  logging: {
    type: "file",
...
```typescript
import { createWorktree } from "@missingstudio/sanddune";
...
const result = await wt.run({
  agent: claudeCode("claude-opus-4-6"),
  sandbox: docker({ imageName: "sanddune:myrepo" }),
  prompt: "Fix issue #42.",
  maxIterations: 3,
});
...
sanddune uses a **branch strategy**, configured per-call on `run()` (and `createWorktree()`), to control how the agent's changes relate to branches. There are three strategies:
...
- **Merge-to-head** (`{ type: "merge-to-head" }`) — sanddune creates a temporary branch in a git worktree. The agent works on the temp branch, and changes are merged back to HEAD when done. The temp branch is cleaned up after merge.
...
sanddune uses a flexible prompt system. You write the prompt, and the engine executes it — no opinions about workflow, task management, or context sources are imposed.
...
> **Convention**: `sanddune init` scaffolds `.sanddune/prompt.md` and all templates explicitly reference it via `promptFile: ".sanddune/prompt.md"`. This is a convention, not an automatic fallback — sanddune does not read `.sanddune/prompt.md` unless you pass it as `promptFile`.
...
!`gh issue list --state open --label sanddune --json number,title,body,comments,labels --limit 20`
...
```typescript
import { run } from "@missingstudio/sanddune";
...
sanddune automatically injects two built-in prompt arguments into every prompt:
...
Select a template during `sanddune init` when prompted, or re-run init in a fresh repo to try a different one.

## CLI commands

### `sanddune init`

Scaffolds the `.sanddune/` config directory and builds the container image. This is the first command you run in a new repo. You choose a sandbox provider (Docker or Podman) during init — selecting Podman writes a `Containerfile` instead of `Dockerfile` and uses `sanddune podman build-image` for the build step.

| Option         | Required | Default                      | Description                                                          |
| -------------- | -------- | ---------------------------- | -------------------------------------------------------------------- |
| `--image-name` | No       | `sanddune:<repo-dir-name>` | Docker image name                                                    |
...
Errors if `.sanddune/` already exists to prevent overwriting customizations.

### `sanddune docker build-image`

Rebuilds the Docker image from an existing `.sanddune/` directory. Use this after modifying the Dockerfile.
...
### `sanddune podman build-image`

Builds the Podman image from an existing `.sanddune/` directory. Use this after modifying the Containerfile.

| Option            | Required | Default                      | Description                                                                          |
| ----------------- | -------- | ---------------------------- | ------------------------------------------------------------------------------------ |
| `--image-name`    | No       | `sanddune:<repo-dir-name>` | Podman image name                                                                    |
| `--containerfile` | No       | —                            | Path to a custom Containerfile (build context will be the current working directory) |

### `sanddune podman remove-image`
...
| `sandbox`            | SandboxProvider    | —                             | **Required.** Sandbox provider (e.g. `docker()`, `podman()`, `docker({ imageName: "sanddune:local" })`)                                                       |
| `cwd`                | string             | `process.cwd()`               | Host repo directory — anchor for `.sanddune/` artifacts and git operations. Relative paths resolve against `process.cwd()`.                                   |
| `prompt`             | string             | —                             | Inline prompt (mutually exclusive with `promptFile`)                                                                                                            |
...
After each Claude Code iteration, sanddune automatically captures the agent's session JSONL from the sandbox to the host at `~/.claude/projects/<encoded-path>/sessions/<session-id>.jsonl`. The `cwd` fields inside each JSONL entry are rewritten to match the host repo root, so `claude --resume` works natively.
...
Before the sandbox starts, sanddune validates that the session file exists on the host and transfers it into the sandbox with `cwd` fields rewritten to match the sandbox-side path. The Claude Code agent receives `--resume <id>` on its print command for iteration 1.
...
Both **agent providers** and **sandbox providers** accept an optional `env: Record<string, string>` in their options. These environment variables are merged with the `.sanddune/.env` resolver output at launch time:
...
- Provider env (agent + sandbox) overrides `.sanddune/.env` resolver output for shared keys
...
Environment variables are also resolved automatically from `.sanddune/.env` and `process.env` — no need to pass them to the API. The required variables depend on the **agent provider** (see `sanddune init` output for details).

## Custom Sandbox Providers

sanddune ships with built-in providers for Docker, Podman, and Vercel, but you can create your own. A sandbox provider tells sanddune how to execute commands in an isolated environment. There are two kinds:

- **Bind-mount** — the sandbox can mount a host directory. sanddune creates a worktree on the host and the provider mounts it in. No file sync needed. Use this for Docker, Podman, or any local container runtime.
          ...
          if (opts?.onLine) {
            ...
            return new Promise((resolve, reject) => {
              ...
              rl.on("line", (line) => {
                ...
                onLine(line); // forward each line to sanddune
              });
...
A branch strategy controls where the agent's commits land. Configure it via the `branchStrategy` option on `run()` (or `createWorktree()`); when omitted, sanddune picks a per-provider default. See [ADR-0009](docs/adr/0009-branch-strategy-per-call.md) for the rationale.
...
| `merge-to-head` | sanddune creates a temp branch, merges back to HEAD when done          | Supported  | Default   |
...
### Config directory (`.sanddune/`)

All per-repo sandbox configuration lives in `.sanddune/`. Run `sanddune init` to create it.

### Custom Dockerfile

The `.sanddune/Dockerfile` controls the sandbox environment. The default template installs:
