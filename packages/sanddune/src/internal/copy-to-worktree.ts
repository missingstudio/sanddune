...
/** Items resolve relative to `cwd` (target-repo perspective), not
 *  process.cwd(). Rejected for `head` strategy. */
export async function runCopyToWorktree(input: {
  ...
  // Read by the timeout factory at fire time so the error names the item
