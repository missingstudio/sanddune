...
// We pass `git -c user.email=... -c user.name=...` inline rather than
// relying on the Dockerfile's global config, because resolveEnv currently
// propagates the host's `HOME=/Users/...` into the container, shadowing the
// container's `~/.gitconfig`. (Tracked as a follow-up; see PR description.)
const shellAgent = createAgentProvider({
  name: "shell-agent",
  buildCommand: () => {
    const gitId =
      "git -c user.email=agent@sanddune.local -c user.name='sanddune agent' -c commit.gpgsign=false";
    return [
      "echo 'hello from merge-to-head (shell agent)' > MERGE-TO-HEAD-SHELL.md",
      `${gitId} add MERGE-TO-HEAD-SHELL.md`,
      `${gitId} commit -m 'add MERGE-TO-HEAD-SHELL.md'`,
    ].join(" && ");
  },
  parseLine: () => [],
});
