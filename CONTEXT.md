...
**Handle-bound run**:
The per-call orchestration that drives one bounded **iteration loop** against a live **sandbox** handle and assembles a `RunResult`. Owns: the pre-loop `beforeSha` capture, **agent invoker** layer construction (with the test-seam fallback), **session capture** wiring, the `runIterationLoop` dispatch, and `RunResult` assembly. Does NOT own: the **run session** lifecycle (the caller opens and closes it so a `runStarted` / `runEnded` record still surrounds sandbox-creation errors), **sandbox** lifecycle, **worktree** lifecycle, lifecycle hooks, **prompt pipeline** preparation (the caller prepares so a bad **prompt template** fails before any sandbox spins up), or `strategy.finalize()`. Used by `runProgram` (constructs the handle fresh per call, optionally passes `resumeSessionId` after `transferSessionToSandbox` succeeds) and by `Sandbox.run` (reuses an existing handle, never accepts `resumeSessionId`).
_Avoid_: "iteration session" (overlaps with **run session**), "run body" (too vague), "sandbox-bound run"
...
- At launch, sanddune resolves env vars from `.sanddune/.env`, **agent provider** `env`, **sandbox provider** `env`, and `RunOptions.env`, then passes the resolved declared keys into the **sandbox**. A key reaches the **sandbox** iff it has been declared in one of those sites; `process.env` is consulted only as a value source for already-declared keys. See ADR-0012 for the rationale behind the declaration-driven model
- Env vars come from four sources, layered (lowest -> highest precedence): (1) `process.env` -- value source only, never declares; (2) `.sanddune/.env` -- declaration site and value source, read by sanddune at launch; (3) **agent provider** `env` and **sandbox provider** `env` -- declaration sites and value sources, must be disjoint (overlap throws at launch, since neither provider has authority over a shared key); (4) `RunOptions.env`, the call-site escape hatch -- declaration site, value source, and override, allowed to overlap with provider env. A declared key with no value in any layer is dropped from the resolved env. Per-provider `env` exists for type-level credential declarations (e.g. **agent providers** can require `ANTHROPIC_API_KEY` at compile time); `RunOptions.env` exists for arbitrary call-site overrides without forcing the user to pick a provider to "own" the key
